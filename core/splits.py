...
def replace_splits(conn, transaction_id: int, lines: list[dict], locked: bool = True) -> None:
    ...
    for line in lines:
        conn.execute(
            "INSERT INTO splits (transaction_id, amount, category_id, person_id, is_splitwise, note, locked) "
            "VALUES (?, ?, ?, ?, ?, ?, ?)",
            (
                transaction_id,
                line["amount"],
...
def set_single_category(conn, transaction_id: int, category_id: int) -> None:
    """Quick path for the common case: the whole transaction is one category,
    no split needed. Overwrites any existing split lines."""
    txn = get_transaction(conn, transaction_id)
    replace_splits(conn, transaction_id, [{"amount": txn["amount"], "category_id": category_id}])
